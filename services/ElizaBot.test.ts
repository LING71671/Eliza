...
describe('ElizaBot', () => {
  ...
  describe('English Interactions', () => {
    ...
    it('should handle fallbacks', () => {
      // Something that definitely won't match any englishScript patterns except the catch-all
      ...
      const allPossibleFallbacks = [...englishScript.fallbacks, ...catchAllResponses];
  ...
  describe('Chinese Interactions', () => {
    ...
    it('should match Chinese fallbacks or catch-all rules', () => {
      const input = "随便乱说的没有任何匹配";
      const response = bot.processInput(input);

      const catchAllRule = chineseScript.keywords.find(k => k.pattern.toString() === '/(.*)/i');
      const catchAllResponses = catchAllRule?.responses.map(r => r.replace('{0}', input)) || [];
      const allPossibleFallbacks = [...chineseScript.fallbacks, ...catchAllResponses];
