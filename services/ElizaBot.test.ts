...
describe('ElizaBot', () => {
  ...
  describe('English Interactions', () => {
    ...
    it('should handle fallbacks', () => {
      ...
      const catchAllResponses = catchAllRule ? catchAllRule.responses.map(res => res.replace('{0}', input)) : [];
      const validResponses = [...englishScript.fallbacks, ...catchAllResponses];
  ...
  describe('Chinese Interactions', () => {
    ...
    it('should match Chinese fallbacks or some generic rule like "我"', () => {
      ...
      const catchAllResponses = catchAllRule ? catchAllRule.responses.map(res => res.replace('{0}', input)) : [];
      const validResponses = [...chineseScript.fallbacks, ...catchAllResponses];
