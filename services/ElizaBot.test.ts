...
describe('ElizaBot', () => {
  ...
  describe('English Interactions', () => {
    ...
    it('should handle fallbacks', () => {
      ...
      const formattedCatchAllResponses = (catchAllRule?.responses || []).map(r => r.replace('{0}', input));
      const allPossibleResponses = [...englishScript.fallbacks, ...formattedCatchAllResponses];
  ...
  describe('Chinese Interactions', () => {
    ...
    it('should handle Chinese fallbacks', () => {
      const input = "随机字符串没有任何匹配"; // Avoid common keywords like "我", "是", "不"
      const response = bot.processInput(input);

      const catchAllRule = chineseScript.keywords.find(k => k.pattern.toString() === '/(.*)/i');
      const formattedCatchAllResponses = (catchAllRule?.responses || []).map(r => r.replace('{0}', input));
      const allPossibleResponses = [...chineseScript.fallbacks, ...formattedCatchAllResponses];
