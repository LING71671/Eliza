...
describe('ElizaBot', () => {
  ...
  describe('English Interactions', () => {
    ...
    it('should handle fallbacks', () => {
      ...
      const catchAllPattern = englishScript.keywords.find(k => k.pattern.toString() === '/(.*)/i');
      const catchAllResponses = catchAllPattern ? catchAllPattern.responses.map(r => r.replace('{0}', input)) : [];
  ...
  describe('Chinese Interactions', () => {
    ...
    it('should match Chinese fallbacks or some generic rule', () => {
      const input = "今天天气真好";
      const response = bot.processInput(input);
      const catchAllPattern = chineseScript.keywords.find(k => k.pattern.toString() === '/(.*)/i');
      const catchAllResponses = catchAllPattern ? catchAllPattern.responses.map(r => r.replace('{0}', input)) : [];
